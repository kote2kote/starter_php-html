  ...
  phpServer.server(
    ...
    function () {
      browserSync.init({
        ...
        proxy: `127.0.0.1:${options.config.port}`,
        port: options.config.port,
      });
    }
  );
...
function icon(done) {
  for (let icon of options.icons) {
    let width = icon[0];
    let height = icon[1];
    src(`${options.paths.src.base}/favicon.png`)
      .pipe(
        imageResize({
          width,
          height,
...
function devPHP() {
  return src(`${options.paths.src.base}/**/*.php`).pipe(dest(options.paths.dist.base));
}
...
function watchFiles() {
  ...
  watch(`${options.paths.src.base}/**/*.php`, series(devPHP, devStyles, previewReload));
  watch(
...
function prodPHP() {
  return src(`${options.paths.src.base}/**/*.php`).pipe(dest(options.paths.build.base));
}
...
exports.default = series(
  ...
  parallel(devStyles, devScripts, devImages, icon, devHTML, devPHP, devManifest), //Run All tasks in parallel
...
exports.prod = series(
  ...
  parallel(prodStyles, prodScripts, prodImages, icon, prodHTML, prodPHP, prodManifest), //Run All tasks in parallel
  buildFinish
);
